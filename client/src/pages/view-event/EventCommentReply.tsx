...
const EventCommentReply = (props: {
  ...
  const [isLikingCommentError, setIsLikingCommentError] =
    useState<boolean>(false);
  ...
  const removeCommentAlertHandler = (
    ...
    <div
      className={
        props.commentReplyId
          ? classes.commentReplyContainer
          : classes.commentContainer
      }
    >
      <div className={classes.nameDate}>
        <p className={classes.userName}>
          <Link to={`/profiles/${props.commentCreatorId}`}>
            {props.commentCreatorUsername}
          </Link>
        </p>
        <p className={classes.userDate}>
          {props.commentCreatedAt
            ? props.commentCreatedAt.split("T")[0]
            : "some time ago..."}
        </p>
      </div>
      <div className={classes.userCommentDeleteContainer}>
        <p className={classes.userComment}>
          {props.commentReplyId ? (
            <span className={classes.commentReplyAt}>
              @{props.commentReplyIdUser}{" "}
            </span>
          ) : (
            ""
          )}{" "}
      ...
      <div className={classes.btnLikeContainer}>
        <div
          ...
          onClick={() =>
            likeCommentHandler({ commentId: props.commentId, userId: user.id })
          }
        >
          {isCommentLiked ? (
            ...
            <CustomTooltip arrow title="Like">
              {user.id ? (
                <ThumbUpIcon
                  fontSize="small"
                  className={classes.thumbUpOutline}
                />
              ) : (
                <Link to="/signup">
                  <ThumbUpIcon
        ...
        <p className={classes.commentLikesCount}>
          {tempCommentLikeCount === 1
            ? `${tempCommentLikeCount} like`
            : `${tempCommentLikeCount} likes`}
        </p>
      </div>
      <CustomSnackBar open={isDeletingCommentError} autoHideDuration={6000}>
        <Alert
          onClose={removeCommentAlertHandler}
          severity="error"
