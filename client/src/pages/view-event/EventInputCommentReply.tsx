...
const EventInputCommentReply = (props: any) => {
  ...
  const removeEventCommentDeleteErrorHandler = (event?: React.SyntheticEvent, reason?: string) => {
    if (reason === 'clickaway') {
      ...
      await axios
        ...
        .then((res: any) => {
          ...
          setTempCommentReplies((comment: any) =>
            comment.concat({ comment: res.data.comment, username: user.username })
          )
        })
  ...
  return (
    <div>
      ...
      <CustomSnackBar open={isDeletingCommentError} autoHideDuration={6000}>
        <Alert onClose={removeEventCommentDeleteErrorHandler} severity="error" className={classes.eventCommentSnackbar}>
