...
async function EditEventImg({ eventId, eventImg }: { eventId: string; eventImg: string }): Promise<IEvent | null> {
  return new Promise((resolve, reject) => {
    try {
      Event.findByIdAndUpdate(eventId, {
        eventImg: eventImg,
...
async function DeleteEventImg({ eventId }: { eventId: string }): Promise<IEvent | null> {
  return new Promise((resolve, reject) => {
    try {
      Event.findByIdAndUpdate(eventId, {
        eventImg: '',
...
async function FindEventNameSearch(search: string) {
  return new Promise((resolve, reject) => {
    ...
    const escapeRegExp = search.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, '\\$&')
    ...
    Event.find({ $or: [{ eventName: searchEvent }, { eventCity: searchEvent }] })
      .limit(100)
      .exec((err, res) => {
        if (err) {
          reject(err)
...
async function DeleteEvent({ eventId }: { eventId: string }): Promise<IEvent | null> {
  return new Promise((resolve, reject) => {
...
async function DeleteEventComment({ commentId, eventId }: { commentId: string; eventId: string }) {
  return new Promise((resolve, reject) => {
    Event.findById(eventId, (err, event) => {
      ...
      Event.findByIdAndUpdate(eventId, { eventComments: eventArr }).exec((err, res) => {
        if (err) {
          reject(err)
        } else {
          if (res) {
  ...
  return new Promise((resolve, reject) => {
    Event.findById(eventId, (err, event) => {
      ...
      if (event?.usersAttending.includes(userId)) {
        ...
        Event.findByIdAndUpdate(eventId, { usersAttending: filterUsersAttending }).exec((err, res) => {
          if (err) {
            reject(err)
          } else {
            if (res) {
        ...
        Event.findByIdAndUpdate(eventId, { usersAttending: [...event?.usersAttending, userId] }).exec((err, res) => {
          if (err) {
            reject(err)
          } else {
            if (res) {
  ...
  return new Promise((resolve, reject) => {
    Event.findById(eventId, (err, event) => {
      if (event?.eventLikes.includes(userId)) {
        ...
        // Set the new array with the removed userId
        Event.findByIdAndUpdate(eventId, { eventLikes: filterEventLikes }).exec((err, res) => {
          if (err) {
            reject(err)
          } else {
            if (res) {
        ...
        Event.findByIdAndUpdate(eventId, { eventLikes: [...event?.eventLikes, userId] }).exec((err, res) => {
          if (err) {
            reject(err)
          } else {
            if (res) {
  ...
  return new Promise((resolve, reject) => {
    try {
      Event.findById(eventId).then((findEvent) => {
        Event.findByIdAndUpdate(eventId, { eventComments: [...findEvent?.eventComments, commentId] }).exec(
          (err, res) => {
            if (err) {
              reject(err)
            } else {
